...
const byte BATTERY_MIN_VOLTAGE = 5;
const byte BATTERY_NUMBER_OF_CELLS = 2;

const float BATTERY_CELL_DEFAULT_VOLTAGE = 4.2;
const float BATTERY_CELL_WARN_VOLTAGE = 3.5;

const float BATTERY_DEFAULT_VOLTAGE = BATTERY_CELL_DEFAULT_VOLTAGE * BATTERY_NUMBER_OF_CELLS;
const float BATTERY_WARN_VOLTAGE = BATTERY_CELL_WARN_VOLTAGE * BATTERY_NUMBER_OF_CELLS;
...
float batteryValue = BATTERY_DEFAULT_VOLTAGE;
...
void measureBattery() {
  ...
  batteryValue = BATTERY_DIVISOR_MULTIPLIER * sensorValue * (3.3 / 1023.0);
  ...
  batteryState = ok;
  if (batteryValue < BATTERY_MIN_VOLTAGE) {
    batteryState = off;
  } else if (batteryValue < BATTERY_WARN_VOLTAGE) {
    batteryState = low;
    #ifdef FREE_SPEK_DEBUG
...
void writeCurrentValues() {
  ...
  **/    
  ...
  #ifdef DSMX
  // set DSMX header flag (bit 11)
  dsmHeader[0] |= (1 << 3);
  #endif

  #ifndef FRANCE
  // set normal send header
  dsmHeader[0] |= (1 << 4);
  #endif
...
void setup() {   
  ...
  measureBattery();
  timer.every(CHECK_BATTERY_INTERVAL, &measureBattery);
