...
//
//
//  LCD display layout ideas
//   0123456789012345
//  .----------------.  .----------------.  .----------------. 
// 0|lat:+40,42,34.64|  |Rec    hh:dd:ss*|  |lat:+40,42,34.64|
// 1|lon:-74,00,32.59|  |w:xxxx,yyyy,zzzz|  |lon:-74,00,32.59|
//  '----------------'  '----------------'  '----------------'
//
...
void loop()
{
    ...
    if( (thistime - lasttime) >= sensorUpdateMillisWFudge ) { 
        ...
        // Do UI Parsing
        //loggingDisabled = wiichuck_cbutton();
        ...
        if( wiichuck_joyy() > 0xA0 ) {
            disp_mode = DISP_MAX;
        } else if( wiichuck_joyy() < 0x40 ) {
            disp_mode = DISP_MIN;
        } else {
            disp_mode = DISP_REC;
        }
